...
>
> データベースは **PostgreSQL 16** を使用する([技術スタック.md](技術スタック.md) 参照)。§7 に実際の DDL(CREATE TABLE / CREATE INDEX)を掲載する。
...
| boards は論理削除 | `boards` テーブルのみ `deleted_at TIMESTAMPTZ` による論理削除。配下の lists / cards は物理削除のまま(FK CASCADE)。`deleted_at IS NULL` が有効なボード。 | ボード復元を可能にするため (V3 マイグレーションで追加)。 |
...
| `deleted_at` | TIMESTAMPTZ | YES | NULL | 論理削除日時。NULL = 有効なボード。V3 マイグレーションで追加。 |

#### 制約
- `CONSTRAINT pk_boards PRIMARY KEY (id)`
- `CONSTRAINT ck_boards_title CHECK (length(trim(title)) > 0)`
- `INDEX idx_boards_deleted_at ON boards (deleted_at) WHERE deleted_at IS NULL` — 有効ボードの検索を高速化（部分インデックス）。V3 マイグレーションで追加。
