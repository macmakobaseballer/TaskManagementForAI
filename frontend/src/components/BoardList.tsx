    ...
    <main className="p-8">
      ...
      <div className="flex flex-wrap gap-4">
        {boards.map((board) => (
          <div
            key={board.id}
            className="relative group w-72 h-44 rounded-xl overflow-hidden shadow-md ring-1 ring-white/40 hover:shadow-lg transition-shadow"
          >
            {/* カード本体（クリックで遷移） */}
            <button
            ...
            >
              ...
              <div className="flex-1 bg-white/70 backdrop-blur-md px-3 py-2.5 flex items-start">
                {renamingBoardId === board.id ? (
                  ...
                  <span className="font-bold text-gray-800 text-lg leading-snug text-left">{board.title}</span>
                )}
              </div>
              {/* 下部: メタ情報エリア */}
              <div className="bg-white/50 backdrop-blur-md px-3 py-2.5 flex flex-col gap-1 text-sm text-gray-700 border-t border-white/40">
